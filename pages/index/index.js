      ...
      .get({
        success: res => {
          ...
          res.data.forEach(function(element, index) {
            let arr_discussion;
            // statements get answer and quester 
            console.log('element', element)
            console.log(element.qDescription,element.crop,element.brows,element.data,element.qImgs)
            arr_discussion.qDescription = element.qDescription
            arr_discussion.crop = element.crop
            arr_discussion.brows = element.brows
            arr_discussion.date = element.data
            arr_discussion.imgs = element.qImgs

            console.log('arr_ele',arr_discussion)
            // get user info
            db.collection('user')
              .limit(1)
              .where({
                ...
                success: res => {
                  // get requset of user infotion 
                  console.log("user", res)
                  let user = res.data[0];
                  arr_discussion[index].name = user.name;
                  arr_discussion[index].avater = user.avater;
                  arr_discussion[index].name = user.name;
                }
              })

            // get answer discussion
            db.collection('discussionAnswer')
              .limit(1)
              .where({
                _id: element.answerDiscussion
              })
              .get({
                success: res => {
                  console.log('answerDiscussion', res)
                  // get answer of user
                  db.collection('user')
                    .limit(1)
                    .where({
                    ...
                    .get({
                      success: res => {
                        console.log('answer user info => ', res)
                        let answer_user = res[0]
                        arr_discussion[index].name = answer_user.name
                        arr_discussion[index].avater = answer_user.avater
                        arr_discussion[index].date = answer_user.date
                      }
                    })
                }
              })
          ...
          console.log('hello')
          console.log('arr_discussion',arr_discussion)

          // get answer discussion
          db.collection('discussionAnswer')
            .limit(1)
            .where({
              _id: res.answerDiscussion
            })
            .get({
              success: res => {
                console.log('answerDiscussion', res)
